<template>
	<view class="wrap">
		<template v-if="vuex_token">
			<view v-if="goodsList.length" class="cartGoodsList u-p-b-80 u-p-l-30">
			...
			<view class="bottomFixed u-flex u-p-15 u-p-l-30">
				<view class="u-flex-5">
				...
				<view class="u-flex u-flex-7 u-row-around">
					...
					<u-button type="primary" :ripple="true" shape="circle">去结算</u-button>
				</view>
			</view>
		</template>
		<template v-else>
		...
		computed: {
			...
			totalPrice() {
				return this.goodsList.filter((goods) => { //过滤掉没有选中商品
					...
					return (parseInt(pre) + parseInt(goods.goods.price) * goods.num).toFixed(1) //乘商品数量
				}, 0)
			}
		},
		methods: {
